...
def make_request(final_transcription, prompt_type):         
    ...
    if prompt_type == "speech":
        ...
        prompt = "Turn this speech into flashcards."
...
# Move the transcription logic into a separate function or module

@app.route('/start_transcription', methods=['GET'])
def start_transcription():
    
    # You would call an asynchronous function to handle transcription
    # For example, you could use threading or a task queue
    return jsonify({"message": "Transcription started"}), 200

@app.route('/stop_transcription', methods=['GET'])
def stop_transcription():
    # You would signal the transcription to stop
    return jsonify({"message": "Transcription stopped"}), 200

@app.route('/get_transcription', methods=['GET'])
def get_transcription():
    # Retrieve the latest transcription
    # This would typically involve reading from a file or database
    transcription = "Placeholder for transcription"
    return jsonify({"transcription": transcription}), 200

if __name__ == '__main__':
    app.run(debug=True)  # This is the correct way to start the Flask app